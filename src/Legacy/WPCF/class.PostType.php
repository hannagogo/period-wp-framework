...
/* //
// Create PostType:
do_action('WPCF_Add_PostTypes', array(
 apply_filters('WPCF_New_PostType', array(
  'name' => 'post_type_name',
...
// Referencing PostType: 
global $wpcf_post_type
;
$post_type = $wpcf_post_types['custom_post_type_name'];
...
function __construct($atts) {
 ...
 if (isset($atts['custom_rules']) && $atts['custom_rules']) $this->add_custom_rules($atts['custom_rules']);
 $post_type_exists = null;

 if ($wpcf_post_types) {
  foreach ($wpcf_post_types as $k=>$p) {
   if ($p->atts['name'] == $this->atts['name']) { return; $post_type_exists = true; break; }
  }
 }

 /* ////// Common Meta Boxes ////// */
 $this->atts['common_meta_box'] = (array) (isset($atts['common_meta_box']) ? $atts['common_meta_box'] : array())
 ;
 /* ////// Meta Boxes ////// */
 $this->atts['meta_boxes'] = (array) (isset( $atts['meta_boxes'] ) ? $atts['meta_boxes'] : array())
 ;

 if (isset($atts['meta_box'])) { $this->atts['meta_boxes'][] = $atts['meta_box']; }
 ...
 foreach ($this->atts['taxonomies'] as $i=>$tax) {
  if (isset($tax['manage_columns']) && (bool) $tax['manage_columns']) {
   $this->atts['taxonomies'][$i]['manage_columns']
    = apply_filters('WPCF_Parse_Arguments', array($tax['name']=>$tax['label']), $tax['manage_columns'] );
  }
  else {
...
// add_filter('wp_title', array($this, 'post_type_title'), 10, 3);
 ...
 add_action('WPCF', array(&$this, 'enqueue_post_type_scripts_and_styles')); 
 add_action('admin_menu', array(&$this, 'setup_common_meta_box_by_post_type'));
...
function enqueue_post_type_scripts_and_styles() {
 if (is_specific_post_type($this->atts['name'])) {
  $post_type_handle_name = 'post_type_'.$this->atts['name'];
  $post_type_archive_handle_name = 'post_type_'.$this->atts['name'].'_archive';
  $post_type_file_name = TEMPLATE_URL . '/' . $post_type_handle_name;
  $post_type_archive_name = TEMPLATE_URL . '/' . $post_type_handle_name . '_archive';
 
  $css_handles = array(
   register_file('css', $post_type_handle_name, $post_type_file_name.'.css', array('theme-style')) => $post_type_file_name.'.css',
   register_file('css', $post_type_archive_handle_name, $post_type_archive_name.'.css', array('theme-style')) => $post_type_archive_name . '.css'
  );
  $js_handles = array(
   register_file('js', $post_type_handle_name, $post_type_file_name.'.js', array('theme-style')) => $post_type_file_name.'.js',
   register_file('js', $post_type_archive_handle_name, $post_type_archive_name . '.js', array('jquery','jquery.jsutil')) => $post_type_archive_name . '.js'
  );
  foreach($css_handles as $h=>$f) {
   if (file_exists(str_replace(TEMPLATE_URL, TEMPLATEPATH, $f))) enqueue_css_handle($h);
  }
  foreach($js_handles as $h=>$f) {
   if (file_exists(str_replace(TEMPLATE_URL, TEMPLATEPATH, $f))) enqueue_js_library_handle($h);
  }
 }
}
 ...
 if (!empty($array)) {
  foreach ($array as $i=>$a) {
   ...
   if (is_array($a)) { // Suppose $a is arguments for class.Taxonomy
    $tax_name = array_value($a,'name');
    $a['post_type'] = $this->name;
    $this->taxonomies[$tax_name] = ( $t = get_taxonomy($tax_name) ) ? $t : new Taxonomy($a);
    $taxes[] = $tax_name;
   }
...
function setup_common_meta_box_by_post_type() { 
  setup_common_meta_box($this->atts['common_meta_box'],$this->name());
}

function setup_meta_boxes($post = null, $boxes = null) {
 if (!is_array($boxes)) $boxes = $this->atts['meta_boxes']; 
 if (count($boxes)) {
  foreach ($boxes as $i=>$b) {
   if (empty($b)) continue;
   $this->meta_boxes[$b['name']] = new MetaBox(array_merge($b, array('post_type'=>$this->name())));
  }
 }
 return $this;
}
...
function &get_meta_box($name) {
 $r = isset($this->meta_boxes[$name]) ? $this->meta_boxes[$name] : NULL ; return $r;
}
...
function _post_type_args($args = NULL) {
 ...
 $labels_defaults = $wp_custom_functions->parse_args(array(
 ...
 $args_defaults = $wp_custom_functions->parse_args(array(
 ...
 $rewrite_defaults = $wp_custom_functions->parse_args( array(
  'slug' => $this->atts['name'],
 ...
 $args = $wp_custom_functions->parse_args(array( // default values of register_post_type().
...
function dashboard_box_html($args=NULL) {
 global $submenu, $custom_language_domain
 ;
...
(
 ...
 $args = parse_args(array(
  ...
  'recent_posts_title' => __('Recent %s', $custom_language_domain),
 ...
 if (empty($submenu_col)) { return ; }
 $html = apply_filters('CF_HTML', 'ul', 'start', array('class'=>'dashboard_submenu', 'id'=>'dashboard_submenu_'.$this->name()));
 foreach ($submenu_col as $s) {
  if (preg_match('/^edit.php\x3f/', $s[2])) { 
   $s[0] = sprintf(__('List %s', $custom_language_domain), $this->label());
  } 
  $html .= apply_filters('CF_HTML', 'li', array('class'=>'dashboard_submenu_li'), 
   apply_filters('CF_HTML', 'a', array('href'=>$s[2]), $s[0])
  );
 }
 $html .= apply_filters('CF_HTML', 'ul', 'end');
 $posts = get_posts(array('post_type'=>$this->name, 'posts_per_page'=>$args['posts_per_page']));
 if (!empty($posts)) {
  $html .= apply_filters('CF_HTML', 'ul', 'start', array('class'=>'dashboard_submenu_posts', 'id'=>'dashboard_submenu_posts_'.$this->name()));
  $html .= apply_filters('CF_HTML', 'li', array('class'=>'dashboard_submenu_posts_li dashboard_submenu_posts_title'),
   sprintf($args['recent_posts_title'], $this->label())
  );
  foreach ($posts as $p) {
   $html .= apply_filters('CF_HTML', 'li', array('class'=>'dashboard_submenu_posts_li'), 
    apply_filters('CF_HTML', 'a', array('href'=>get_edit_post_link($p->ID)), $p->post_title)
   );
  }
  $html .= apply_filters('CF_HTML', 'ul', 'end');
 }
 echo apply_filters('CF_HTML', 'div', array('id'=>'dashboard_submenu_box_'.$this->name().'_content', 'class'=>'dashboard_submenu_box_content'),
  $html
  . apply_filters('CF_Wrap_JavaScript', 
'
...
function post_type_title($title, $sep, $seplocation) {
 return $title; global $wp_query;
 if (!is_custom_post_type_archive() || ($wp_query->query['post_type'] != $this->atts['name'])) return $title;
 $t_sep = '%WP_TITILE_SEP%'; // Temporary separator, for accurate flipping, if necessary
 $title_array = (!empty($title))? array('') : array();
 $title_array = array_merge($title_array, array($this->atts['post_type_args']['label'], $title), explode($t_sep, $title));
 ('right' == $seplocation) && ($title_array = array_reverse($title_array));
 return implode($sep, $title_array);
}


function name() { return $this->atts['name']; }
function label() { return $this->atts['post_type_args']['label']; }
